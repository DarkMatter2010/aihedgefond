"""Manual live feature-class IC triage; excluded from pytest/CI (Yahoo).
...
def main() -> None:
    """Fetch broad universe once, run feature-class IC triage, print report."""
    _enable_insecure_yfinance_ssl_if_needed()
    base = load_settings()
    # Keep limits.yaml train_end/test_start/seed; triage extends test_start per horizon.
    ...
    print("=== Feature-class IC triage (Reversal / Low-Vol / Range) ===")
    ...
    print(f"configs: {[label for label, _ in FEATURE_CLASS_CONFIGS]}")
    print(f"threshold Rank-IC: {RANK_IC_MATERIAL_THRESHOLD}")
    print(f"survivorship: {SURVIVORSHIP_BIAS_NOTE}")
    print(f"reversal_sign: {REVERSAL_SIGN_NOTE}")
    ...
    # Align universe to downloaded symbols before triage.
    run_settings = settings.model_copy(update={"universe": kept})
    report = run_feature_class_triage(bars, run_settings)

    print("--- OOS metrics (Klasse x Horizont) ---")
    print(
    ...
    print(
        f"best: {report.best_class_label} h={report.best_horizon} Rank-IC={report.best_rank_ic:.6f}"
    )
    ...
    print("=== HANDOFF ===")
    print(
        "modules: aihedgefund.features.feature_classes, aihedgefund.research.feature_class_triage"
    )
    print("script: scripts/run_feature_class_triage.py")
    print("next: gate if candidate_for_gate else pipeline Durchstich")
